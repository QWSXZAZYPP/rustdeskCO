...
def make_parser():
    ...
    parser.add_argument(
        '--cli',
        action='store_true',
        help='Build command line interface version without UI'
    )
    # Add static MSVC linking option
    parser.add_argument(
        '--static-msvc',
        action='store_true',
        help='Enable static linking with MSVC runtime (Windows only)'
    )
    return parser
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        ...
        cargo_cmd = f'cargo build --features {features} --lib --release'
        if args.static_msvc:
            cargo_cmd = f'set RUSTFLAGS=-C target-feature=+crt-static && {cargo_cmd}'
        system2(cargo_cmd)
...
def main():
    ...
    if windows:
        ...
        # Add static linking for virtual display if requested
        ...
        # Build command line version or GUI version
        cargo_cmd = 'cargo build --release'
        if args.cli:
            # For CLI version, disable default features and enable cli feature
            cargo_cmd += ' --no-default-features'
            if features:
                cargo_cmd += f' --features {features}'
            else:
                cargo_cmd += ' --features cli'
        else:
            if features:
                cargo_cmd += f' --features {features}'
        ...
        # Rename executable for CLI version
        if args.cli:
            if os.path.exists('target/release/rustdesk.exe'):
                os.rename('target/release/rustdesk.exe', 'target/release/rustdesk-cli.exe')
                print("Built CLI version: target/release/rustdesk-cli.exe")
            else:
                print("Error: CLI executable not found after build")
                sys.exit(1)
        else:
        ...
        if flutter:
            ...
            # Build command line version or GUI version for Linux
            cargo_cmd = 'cargo build --release'
            if args.cli:
                cargo_cmd += ' --no-default-features'
                if features:
        ...
        # Build command line version or GUI version for RHEL/CentOS
        cargo_cmd = 'cargo build --release'
        if args.cli:
            cargo_cmd += ' --no-default-features'
            if features:
        ...
        # Build command line version or GUI version for SUSE
        cargo_cmd = 'cargo build --release'
        if args.cli:
            cargo_cmd += ' --no-default-features'
            if features:
        ...
        if flutter:
            ...
            # Build command line version or GUI version for other systems
            cargo_cmd = 'cargo bundle --release'
            if args.cli:
                cargo_cmd += ' --no-default-features'
                if features:
                    cargo_cmd += f' --features {features}'
